...
class AjaxController extends Controller
{
    ...
    public function interestProvisionList(Request $request)
    {
        ...
        if ($invTypId && $fiscalYearId && ($intProvSearchBtnVal == YesNoFlag::YES) ) {
            ...
            if ($response['o_status_code'] == 1 ){
                $intProvList = DB::select("select * from table (sbc_dev.fas_cm_trans.get_fdr_provision_process_list(:p_investment_type_id,:p_fiscal_year_id))",[ 'p_investment_type_id' =>$invTypId, 'p_fiscal_year_id' => $fiscalYearId ]);
            }
            /*if ($response['o_status_code'] != 1 ){
                $statusCode = $response['o_status_code'];
                $statusMessage = $response['o_status_message'];
    ...
    public function interestProvisionTransViewList(Request $request)
    {
        ...
        if ($invTypId && $fiscalYearId && ($paramVal == YesNoFlag::YES) ) {
            ...
            $intProvTransViewList = DB::select("select * from table (sbc_dev.fas_cm_trans.get_fdr_provision_trans_preview(:p_investment_type_id,:p_fiscal_year_id))",[ 'p_investment_type_id' =>$invTypId, 'p_fiscal_year_id' => $fiscalYearId ]);
    ...
    public function checkFdrProvisionProcess (Request $request)
    {
        ...
        try {
            ...
            DB::executeProcedure('sbc_dev.fas_cm_trans.check_fdr_provision_proccess', $params);
    ...
    public function fdrInvestmentList(Request $request)
    {
        ...
        $bank_id = ($request->post('bankId', NULL) == 'Infinity' ? null : $request->post('bankId', NULL));
        $branch_id = $request->post('branchId', NULL);
    ...
    public function fdrMaturityList(Request $request)
    {
        ...
        $investment_list = DB::select('select sbcacc.fas_cm_trans.get_fdr_investment_maturity_list(:p_investment_type_id,:p_transaction_type,:p_document_date,:p_bank_code,:p_branch_code) from dual',
    ...
    public function fdrInvestmentDetails(Request $request)
    {
        ...
        try {
            ...
            $accountInfo = DB::selectOne("select * from sbcacc.glGetAccountInfo (:p_gl_acc_id,DEFAULT)",
    ...
    public function fdrMaturityDetails(Request $request)
    {
        ...
        $maturity_info = DB::selectOne("select sbcacc.fas_cm_trans.get_fdr_investment_maturity_view(:p_investment_id) from dual",["p_investment_id"=>$fdr_id]);
        ...
        try {
            ...
            $maturity_info->renewal_date = HelperClass::dateConvert($maturity_info->renewal_date);
            $maturity_info->renewal_maturity_date = HelperClass::dateConvert($maturity_info->renewal_maturity_date);
            ...
            return response()->json(['status_code'=>'1',"fdr_info"=>$maturity_info,"acc_info"=>$accountInfo,'contra_acc_list'=>$contraAccs]);
        }catch (\Exception $e){
            return response()->json(['status_code'=>'99',"fdr_info"=>[],"acc_info"=>[],'contra_acc_list'=>[]]);
        }

...
class FdrMaturityAuthorizeController extends Controller
{
    ...
    public function approvalView(Request $request, $id, $filter=null)
    {
        ...
        $fdrMaturityTransView = DB::selectOne("select sbcacc.fas_cm_trans.get_fdr_maturity_trans_view (:p_maturity_trans_id) from dual",['p_maturity_trans_id' => $id]);

        $fdrContraView = DB::selectOne("select sbcacc.fas_cm_trans.get_fdr_investment_contra_ac (:p_investment_type_id) from dual",['p_investment_type_id' => $fdrMaturityTransView->investment_type_id]);

        $contraBankAccountInfo = DB::selectOne("select sbcacc.fas_gl_trans.get_gl_account_info (:p_gl_acc_id) from dual", ['p_gl_acc_id' => $fdrContraView->gl_acc_id]);

        $fdrMaturityTransViewList = DB::select("select sbcacc.fas_cm_trans.get_gl_transaction_view (:p_gl_trans_master_id) from dual",['p_gl_trans_master_id' => $fdrMaturityTransView->gl_trans_master_id]);

        $fdrMaturityViewSplitList = DB::select("select sbcacc.fas_cm_trans.get_fdr_investment_split_list_view (:p_maturity_trans_id) from dual",['p_maturity_trans_id' => $id]);
    ...
    public function searchFdrIntProvProcAuthorize(Request $request)
    {
        ...
        $sql ="select sbcacc.fas_cm_trans.get_fdr_maturity_trans_auth_list (:p_investment_type_id,:p_fiscal_year_id,:p_posting_period_id,:p_workflow_approval_status,:p_user_id) from dual";

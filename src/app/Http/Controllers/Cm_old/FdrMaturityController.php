...
class FdrMaturityController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::executeProcedure('sbcacc.fas_cm_trans$fas_fdr_maturity_trans_make', $params);
            ...
            if ($status_code != "1") {
                ...
                if ($request->post('transaction_type') == LFdrMaturityTransType::RENEWAL_AND_SPLIT) {
                    foreach ($request->post('line') as $data) {
                        try {
                            ...
                            DB::executeProcedure('sbcacc.fas_cm_trans$fas_fdr_investment_split_save', $params2);
                        } catch (\Exception $e) {
                            DB::rollBack();
                            if (isset($id)) {
                ...
                DB::executeProcedure('sbcacc.fas_workflow$WORKFLOW_MAPPING_ENTRY', $wkMappingParams);
            ...
            DB::commit();
            if (isset($id)) {
                return ["response_code" => $status_code, "response_msg" => $status_message,"trans_type"=>$params['p_transaction_type'],"investment_type"=>$params['p_investment_id']];
            } else {
                return response()->json(["response_code" => $status_code, "response_msg" => $status_message,"trans_type"=>$params['p_transaction_type'],"investment_id"=>$params['p_investment_id']]);
            }
        } catch (\Exception $e) {
            DB::rollBack();
            if (isset($id)) {
    ...
    public function dataList(Request $request)
    {
        ...
        $maturity_list = DB::select('select sbcacc.fas_cm_trans.get_fdr_maturity_trans_make_list(:p_investment_type_id,:p_fiscal_year_id,:p_posting_period_id,:p_workflow_approval_status) from dual',
...
SELECT * FROM TABLE (fas_cm_trans.get_fdr_maturity_trans_preview (
    ...
    public function view(Request $request, $id)
    {
        ...
        $contraAcc = DB::select("select sbcacc.fas_cm_trans.get_fdr_investment_contra_ac (:p_investment_type_id) from dual",
        ...
        return view('cm.fdr-maturity.view',compact(  'transactionContents','contraAcc','periodTypes', 'investmentTypes', 'fiscalYear','transaction','splitInfo' ));
    }
}

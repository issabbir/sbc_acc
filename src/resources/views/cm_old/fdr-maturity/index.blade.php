        ...
        function fdrModalActions() {
            ...
            function setFdrMatureInfo(data) {
                ...
                if (data.status_code != 99) {
                    ...
                    $("#last_renewal_date_field").val(data.fdr_info.renewal_date);
                    $("#last_renewal_amount").val(data.fdr_info.renewal_amount);
                    $("#last_maturity_date_field").val(data.fdr_info.renewal_maturity_date);
                    $("#last_interest_rate").val(data.fdr_info.renewal_interest_rate);
                        ...
                        request.done(function (res) {
                            if (res.response_code == "1") {
                                Swal.fire({
                                    ...
                                    $('#print_btn').html('<a target="_blank" href="{{request()->root()}}/report/render/RPT_CHALLAN?xdo=/~weblogic/FAS_NEW/CASH_MANAGEMENT/RPT_CHALLAN.xdo&p_maturity_trans_type_id=' + res.trans_type + '&p_investment_id=' + res.investment_id + '&type=pdf&filename=maturity_chalan_report"  class="cursor-pointer btn btn-sm btn-info"><i class="bx bx-printer font-size-small"></i>Print Last Chalan</a>');

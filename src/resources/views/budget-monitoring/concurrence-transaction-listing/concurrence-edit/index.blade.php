...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            @include('budget-monitoring.concurrence-transaction-listing.concurrence-edit.form')
        </div>
    </div>
        ...
        * */
        ...
        $("#concurrence_form").on("submit", function (e) {
            ...
            swal.fire({
                ...
                if (result.value == true) {
                    $('<input type="hidden" name="mode" value="{{\Illuminate\Support\Facades\Crypt::encryptString('E')}}">').appendTo('#concurrence_form');

                    let data = new FormData(this);
                    data.set('est_amount',removeCommaFromValue(data.get('est_amount')))
                    data.set('balance_amount',removeCommaFromValue(data.get('balance_amount')))
        ...
        $(document).ready(function () {
            ...
            if (!nullEmptyUndefinedChecked($("#budget_booking_id").val()) && $("#tender_proposal_type").select2().val() == '{{\App\Enums\BudgetMonitoring\TenderType::ADVANCE}}') {
                ...
                $("#tender_proposal_type option").each(function () {
                    if (($(this).val() != '{{\App\Enums\BudgetMonitoring\TenderType::ADVANCE}}') && ($(this).val() != '{{\App\Enums\BudgetMonitoring\TenderType::ADJUSTMENT}}')){
                        $(this).remove();
                    }
                });
                //0003374: Budget monitoring>Budget Concurrance>Listing>Advance Edit
                //$("#tender_proposal_type").parent('div').addClass('make-readonly');
                //What if user want's to change type from advance to something else!!!!!!!!
                //Ha!! Ha!! Ha!! finally CR came 0003374:
            } else {
